...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    try:
        ...
        s.listen(1)
        print(f"TCP Server is now listening on the specified port {HOST}:{PORT}")

        # Accept a client connection
        conn, addr = s.accept()
        print(f"Connection established through: {addr}")
...
print("Connection Error: The server has closed the connection unexpectedly")
